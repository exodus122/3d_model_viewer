...
// Raised after finding that the real accuracy limiter was the refiner being
// cut short by a spurious XZ-extent stop, not the seed grid. With that
// fixed, cells cost more but resolve properly, so the budget is set to keep
// mid-sized maps at or near scale 1 rather than coarsening them preemptively.
const STANDABLE_ADAPTIVE_PROBE_BUDGET = 400000;
...
function buildStandableSurfaceTriangles(tri, pushGreen, pushRed, pushBlue, pushYellow, colCtx = null, polyIdx = null, budget = null, quality = null, info = null, stats = null) {
    ...
    const bulgeDensity = STANDABLE_ADAPTIVE_EXEMPT_BULGES
        ? makeDensity(1.0)
        : surfaceDensity;
    ...
    const refine = (pushFn, verts, minX, maxX, minZ, maxZ, depth, dens) => {
        ...
        // NOTE: there is deliberately NO XZ-extent stop here.
        //
        // An earlier revision added one ("stop once the cell is smaller than
        // hardFloor in both axes"). It was a mistake and was removed. On a
        // near-vertical triangle the steepness-aware seed step already makes
        // cells tiny in XZ (seedX = SEED * |Ny/Nx| collapses as Ny -> 0), so an
        // XZ stop fires after only a few splits while the cell still spans a
        // large Y range - and the cell is then decided by a single centroid
        // probe. That silently coarsened exactly the steep surfaces this
        // refiner exists to resolve, at every scale including 1.0.
        //
        // The Y span is the quantity that governs the validity boundary, so
        // yThin (below) is the correct and only feature-size terminator; the
        // depth cap is the backstop. The XZ hard floor is still used, but only
        // where it was originally intended - see hardFloor's use for the
        // sub-feature-size bulge pieces via the depth cap.
        ...
        if (yThin || depth >= dens.maxDepth) {
            if (stats) stats.terminalLeaves++;
...
export function renderStandableSurfaceXZ(allTriangleData, colCtx = null) {
    ...
    if (STANDABLE_PERF_LOGGING && typeof console !== 'undefined') {
        ...
        if (stats.probes > 0) {
            ...
            // finer Y floor.
            if (leafRatio < 0.05 && stats.seedCells > 1000) {
                console.warn(
                    `[standable] UNDER-REFINED: only ${(leafRatio * 100).toFixed(1)}% of ` +
                    `refine calls reached a terminal decision (healthy is >10%). ` +
                    `Cells are being decided before their Y band is resolved, so ` +
                    `the surface will look coarse. Scale ${quality.scale.toFixed(2)}x, ` +
                    `yFloor ${quality.yFloor.toFixed(1)}, depth ${quality.maxDepth}, ` +
                    `effective seed step avg ${effAvg.toFixed(1)}` +
