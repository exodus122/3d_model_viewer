...
//
// MEASURED (bulge overshoot vs a near-exact clip, cutoff slicing the bulge):
//   hf 0.25 -> 0.10% overshoot   hf 0.15 -> 0.03%   hf 0.10 -> 0.00%
// and at the worst steepness (Ny=0.02) 0.25 -> 0.35% vs 0.10 -> 0.00%.
// Lowering to 0.10 also REDUCES the thin-triangle fraction there (16.4% ->
// 9.2%), because more of the wedge gets resolved instead of being left as one
// badly-cut piece. Values below 0.10 changed nothing, so this is the knee.
//
// This matters because the bulges have no top clip: pushAboveCutoff only
// bounds them from BELOW (at cutoffY), so on a steep plane a rim point 1 unit
// out in XZ can sit tens of units above the vertex, and only the sampled
// validity test removes it. That sampling has to be fine enough to catch it.
const STANDABLE_OCCLUSION_HARD_FLOOR = 0.10;
...
// MEASURED (overshoot area above a hard cutoff, Ny=0.1):
//   fixed floor 5.67% | 1/4 -> 1.49% | 1/8 -> 0.91% | 1/16 -> 0.88% | 1/32 -> 0.88%
// It saturates at 1/8, so finer values only add triangles.
const STANDABLE_BULGE_Y_FLOOR_FRACTION = 1 / 8;
